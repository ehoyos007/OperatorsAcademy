...
One `Item` record per tool. v1 seed **auto-generated** from `~/Projects/operators-academy-pro` (`skills/*/SKILL.md`, `free-agents/*.md`, `agents/*.md`, `README.md`), then hand-polished. Single source of truth = the toolkit repo; a build script parses it into `src/data/items.json`.
...
- Its data arrays (`baseSkills`, `premiumSkillCategories`, `baseAgents`, `premiumAgents`, plugins — already synced to repo v2.1) **migrate into `items.json`** as the seed. No more hand-maintained arrays in the page.
...
5. `items.json` is generated by a script from `operators-academy-pro` (re-runnable when the toolkit changes).
